...
export const voteModalTexts: LangStringType<VoteModalFuncType> = {
  ...
  es: ({ dailyTicketCount, starName, moreVoteCount }) => {
    return {
      ...
      voteBlockEnd: `Si instalan la aplicación FanPlus se puede obtener <br/> <b>${moreVoteCount}</b> voto(s) cada día`,
    };
  },
  in: ({ dailyTicketCount, starName, moreVoteCount }) => {
    return {
      voteProcess: `Apakah Anda ingin memilih <b>${dailyTicketCount}</b> suara<br/> secara gratis untuk <span>${starName}</span>?`,
      ...
      voteBlockEnd: `FanPlus app Instal aplikasinya.<br/>Anda mendapatkan <br/> <b>${moreVoteCount}</b> suara memberikan suara setiap hari`,
    };
  },
  ...
  vi: ({ dailyTicketCount, starName, moreVoteCount }) => {
    return {
      voteProcess: `Bạn có muốn bình chọn phiếu <b>${dailyTicketCount}</b> miễn<br/> phí cho <span>${starName}</span> không?`,
