...
const handler: NextApiHandler = async (req, res) => {
  const { lang, comment_idx, order_by, identity } = req.query;
  const page = 0;
  const origin = process.env.NEXT_PUBLIC_CLIENT_URL || 'https://dev.fanplus.co.kr';

  try {
    const response: AxiosResponse<CommunityBoardResponseType> = await axios.get(
      `https://napi.appphotocard.com/v1/subcomments?lang=${lang}&page=${page}&comment_idx=${comment_idx}&order_by=${order_by}&identity=${identity}`,
      {
        headers: {
          Origin: origin,
